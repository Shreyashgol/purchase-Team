...
def execute(intent, repository) -> SalesActionResponse:
    ...
    question = intent.fetchQuery or "show sales orders"
    ...
    return SalesActionResponse(
        status="success",
        message=f"Sales order fetch completed successfully. {len(rows)} result(s) returned.",
        data={
            "sql": query_spec["sql"],
