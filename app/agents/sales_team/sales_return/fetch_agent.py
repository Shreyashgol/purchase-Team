...
def execute(intent, repository) -> SalesActionResponse:
    ...
    question = intent.fetchQuery or "show sales returns"
    ...
    return SalesActionResponse(
        status="success",
        message=f"Sales return fetch completed successfully. {len(rows)} result(s) returned.",
        data={
            "sql": query_spec["sql"],
            ...
            "agent": "sales_return.fetch_agent",
        },
    )
